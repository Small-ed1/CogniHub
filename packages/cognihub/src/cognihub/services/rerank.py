...
def _extract_json_array(text: str) -> list[Any] | None:
    ...
    if not s:
        return None
    # Best-effort: find first '[' and last ']' and attempt JSON parse.
    start = s.find("[")
    end = s.rfind("]")
...
async def rerank_results(
    *,
    ...
    This is intentionally best-effort: if anything looks off, fall back to original order.
    """
    if not results:
        return []
    ...
    # Append any candidates the model omitted, in original order.
